...
| Phone layout | One breakpoint, `max-width: 700px`, and every phone rule sits at the **foot** of the stylesheet it belongs to — `theme.css` (tokens, the rotate gate), `game/game.css`, `collection/collection.css`. Nothing above 700px reaches them, so desktop is untouched by construction; keep it that way rather than editing a desktop rule to suit a phone. The rails stop being columns: the left one is a fixed top strip, the right one and the annals are drawers the `game/PhoneDock.tsx` bar raises (`data-sheet` on `.field`), and the turn cue is fixed above the dock. Tap-to-play already worked — `startDrag` sets `held` on pointerdown and `preventDefault` kills the trailing click — so the job is layout, not input. Landscape is refused, not squeezed |
